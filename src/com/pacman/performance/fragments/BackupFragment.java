...
public class BackupFragment extends PreferenceFragment implements Constants {
    ...
    private CustomPreference mBoot, mRecovery, mFota;

    private CustomPreference[] mBackups;
    ...
    private final Runnable run = new Runnable() {
        ...
        public void run() {
            ...
            if (rootHelper.getPartitionName(PartitionType.BOOT) != null) {
                mBoot = new CustomPreference(getActivity(),
                        getString(R.string.boot), getString(
            ...
            if (rootHelper.getPartitionName(PartitionType.RECOVERY) != null) {
                mRecovery = new CustomPreference(getActivity(),
                        getString(R.string.recovery), getString(
                                R.string.backup_summary,
            ...
            if (rootHelper.getPartitionName(PartitionType.FOTA) != null) {
                mFota = new CustomPreference(getActivity(),
                        getString(R.string.fota), getString(
                                R.string.backup_summary,
            ...
            if (backups.length < 1) root.addPreference(new CustomPreference(
                    getActivity(), null, getString(R.string.no_backups)));
            else {
                // Date formater
                DateFormat f = DateFormat.getDateTimeInstance(DateFormat.SHORT,
                        DateFormat.SHORT, Locale.getDefault());

                mBackups = new CustomPreference[backups.length];
                for (int i = 0; i < backups.length; i++) {
                    mBackups[i] = new CustomPreference(getActivity(),
    ...
    public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen,
        ...
        File[] backups = new File(PAC_BACKUP).listFiles();
        for (int i = 0; i < backups.length; i++)
            if (preference == mBackups[i])
                showMenu(mBackups[i].getTitle().toString());
    ...
    private void restoreDialog(final String file) {
        ...
        if (partition.equals(getString(R.string.boot)))
            type = PartitionType.BOOT;
        if (partition.equals(getString(R.string.recovery)))
            type = PartitionType.RECOVERY;
        if (partition.equals(getString(R.string.fota)))
            type = PartitionType.FOTA;
