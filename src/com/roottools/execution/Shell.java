 ...
 */
package com.roottools.execution;
...
public class Shell {
	...
	public void useCWD(Context context) throws IOException, TimeoutException,
			RootDeniedException {
		add(new CommandCapture(-1, false, "cd "
	...
	public String getCommandQueuePositionString(Command cmd) {
		return "Command is in position " + getCommandQueuePosition(cmd)
				+ " currently executing command at position " + this.write
				+ " and the number of commands is " + commands.size();
	}
	...
	public static boolean isAnyShellOpen() {
		return Shell.shell != null || Shell.rootShell != null
				|| Shell.customShell != null;
	}
	 ...
	 * <p/>
	 * When writing commands we stay in a loop and wait for new commands to
	 * added to "commands"
	 * <p/>
	 * The notification of a new command is handled by the method add in this
	 * class
	 */
			...
			try {
				while (true) {
					...
					synchronized (commands) {
						/**
						 * While loop is used in the case that notifyAll is
						 * called and there are still no commands to be written,
						 * a rare case but one that could happen.
						 */
		...
		public void run() {
			try {
				...
				while (!close) {
					 ...
					 * 
					 * if the token is present then the command has finished
					 * execution.
					 */
					...
					if (pos > 0) {
						/**
						 * token is suffix of output, send output part to
						 * implementer
						 */
	...
	public static Shell startShell(int timeout) throws IOException,
			TimeoutException {
		Shell.shellTimeout = timeout;
