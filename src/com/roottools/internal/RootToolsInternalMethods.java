...
package com.roottools.internal;
...
import com.roottools.Constants;
...
public final class RootToolsInternalMethods {
	...
	public static void getInstance() {
		// this will allow RootTools to be the only one to get an instance of
		// this class.
	...
	public boolean deleteFileOrDirectory(String target, boolean remountAsRw) {
		...
		try {
			...
			if (hasUtil("rm", "toolbox")) {
				...
				CommandCapture command = new CommandCapture(0, false, "rm -r "
						+ target);
				Shell.startRootShell().add(command);
				commandWait(Shell.startRootShell(), command);
				...
				if (checkUtil("busybox") && hasUtil("rm", "busybox")) {
					...
					if (command.getExitCode() != 0) {
						RootTools
								.log("target not exist or unable to delete file");
						result = false;
					}
				}
			}
		...
		try {
			...
			if (RootTools.findBinary(util)) {
				...
				for (String path : paths) {
					CommandCapture command = new CommandCapture(0, false,
							utilPath + " rm " + path + "/" + util);
					Shell.startRootShell().add(command);
					commandWait(Shell.startRootShell(), command);

				}

				CommandCapture command = new CommandCapture(0, false, utilPath
						+ " ln -s " + utilPath + " /system/bin/" + util,
	...
	public boolean findBinary(final String binaryName) {
		...
		try {
			for (final String path : places) {
				CommandCapture cc = new CommandCapture(0, false, "stat " + path
						+ binaryName) {
					@Override
					public void commandOutput(int id, String line) {
						if (line.contains("File: ")
							...
							RootTools.log(binaryName + " was found here: "
									+ path);
						}
				...
				if (paths != null) {
					for (String path : paths) {
						if (RootTools.exists(path + "/" + binaryName)) {
							...
							RootTools.log(binaryName + " was NOT found here: "
									+ path);
						}
					}
				}
	...
	public List<String> getBusyBoxApplets(String path) throws Exception {
		...
		CommandCapture command = new CommandCapture(Constants.BBA, false, path
				+ "busybox --list") {
	...
	public String getBusyBoxVersion(String path) {
		...
		try {
			CommandCapture command = new CommandCapture(Constants.BBV, false,
					path + "busybox") {
				@Override
				public void output(int id, String line) {
					if (id == Constants.BBV) {
						if (line.startsWith("BusyBox")
								&& InternalVariables.busyboxVersion.equals("")) {
			...
			RootTools
					.log("BusyBox was not found, more information MAY be available with Debugging on.");
			return "";
		}
	...
	public String getInode(String file) {
		try {
			CommandCapture command = new CommandCapture(Constants.GI, false,
					"/data/local/ls -i " + file) {

				@Override
				public void output(int id, String line) {
					if (id == Constants.GI) {
						if (!line.trim().equals("")
								&& Character.isDigit((char) line.trim()
	...
	public boolean isAccessGiven() {
		try {
			...
			CommandCapture command = new CommandCapture(Constants.IAG, false,
					"id") {
				@Override
				public void output(int id, String line) {
					if (id == Constants.IAG) {
						Set<String> ID = new HashSet<String>(Arrays.asList(line
								.split(" ")));
	...
	public boolean isNativeToolsReady(int nativeToolsId, Context context) {
		...
		if (installer.isBinaryInstalled("nativetools")) {
			...
			InternalVariables.nativeToolsReady = installer.installBinary(
					nativeToolsId, "nativetools", "700");
		}
	...
	public Permissions getFilePermissionsSymlinks(String file) {
		...
		if (RootTools.exists(file)) {
			...
			try {
				...
				CommandCapture command = new CommandCapture(Constants.FPS,
						false, "ls -l " + file, "busybox ls -l " + file,
	...
	public long getSpace(String path) {
		...
		try {
			final CommandCapture command = new CommandCapture(Constants.GS,
					false, "df " + path) {
	...
	public String getSymlink(String file) {
		...
		try {
			...
			CommandCapture command = new CommandCapture(Constants.GSYM, false,
					"ls -l " + file) {
			...
			if (symlink.length > 2 && symlink[symlink.length - 2].equals("->")) {
				...
				if (!symlink[symlink.length - 1].equals("")
						&& !symlink[symlink.length - 1].contains("/")) {
					// We assume that we need to get the path for this symlink
					// as it is probably not absolute.
					findBinary(symlink[symlink.length - 1]);
					if (RootTools.lastFoundBinaryPaths.size() > 0) {
						// We return the first found location.
						final_symlink = RootTools.lastFoundBinaryPaths.get(0)
								+ "/" + symlink[symlink.length - 1];
					} else {
	...
	public ArrayList<Symlink> getSymlinks(String path) throws Exception {
		...
		command = new CommandCapture(0, false, "find " + path
				+ " -type l -exec ls -l {} \\; > /data/local/symlinks.txt");
		Shell.startRootShell().add(command);
		commandWait(Shell.startRootShell(), command);
	 ...
	 */
	public boolean installBinary(Context context, int sourceId,
	...
	public boolean killProcess(final String processName) {
		...
		try {
			...
			CommandCapture command = new CommandCapture(0, false, "ps") {
				...
				public void output(int id, String line) {
					if (line.contains(processName)) {
						Matcher psMatcher = InternalVariables.psPattern
								.matcher(line);
