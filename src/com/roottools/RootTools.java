...
import com.roottools.execution.Command;
import com.roottools.execution.Shell;
import com.roottools.internal.Remounter;
import com.roottools.internal.RootToolsInternalMethods;
import com.roottools.internal.Runner;
...
public final class RootTools {
	...
	/**
	 * This class is the gateway to every functionality within the RootTools
	 * library.The developer should only have access to this class and this
	 * class only.This means that this class should be the only one to be
	 * public.The rest of the classes within this library must not have the
	 * public modifier.
	 * <p/>
	 * All methods and Variables that the developer may need to have access to
	 * should be here.
	 * <p/>
	 * If a method, or a specific functionality, requires a fair amount of code,
	 * or work to be done, then that functionality should probably be moved to
	 * its own class and the call to it done here.For examples of this being
	 * done, look at the remount functionality.
	 */
	...
	public static boolean copyFile(String source, String destination,
			boolean remountAsRw, boolean preserveFileAttributes) {
		return getInternals().copyFile(source, destination, remountAsRw,
	 ...
	 */
	public static boolean deleteFileOrDirectory(String target,
			boolean remountAsRw) {
	...
	/**
	 * This will try and fix a given binary. (This is for Busybox applets or
	 * Toolbox applets) By "fix", I mean it will try and symlink the binary from
	 * either toolbox or Busybox and fix the permissions if the permissions are
	 * not correct.
	 * 
	 * @param util
	 *            Name of the utility to fix.
	 * @param utilPath
	 *            path to the toolbox that provides ln, rm, and chmod. This can
	 *            be a blank string, a path to a binary that will provide these,
	 *            or you can use RootTools.getWorkingToolbox()
	 */
	...
	/**
	 * This will check an array of binaries, determine if they exist and
	 * determine that it has either the permissions 755, 775, or 777. If an
	 * applet is not setup correctly it will try and fix it. (This is for
	 * Busybox applets or Toolbox applets)
	 * 
	 * @param utils
	 *            Name of the utility to check.
	 * @return boolean to indicate whether the operation completed. Note that
	 *         this is not indicative of whether the problem was fixed, just
	 *         that the method did not encounter any exceptions.
	 * @throws Exception
	...
	/**
	 * @param path
	 *            String that represents the path to the Busybox binary you want
	...
	/**
	 * This will return an List of Strings. Each string represents an applet
	 * available from BusyBox.
	 * <p/>
	 * 
	 ...
	 * 
	 * @param path
	 *            Path to the busybox binary that you want the list of applets
	 *            from.
	 * @return <code>null</code> If we cannot return the list of applets.
	 */
	...
	/**
	 * This will open or return, if one is already open, a custom shell, you are
	 ...
	 * @param shellPath
	 *            a <code>String</code> to Indicate the path to the shell that
	 *            you want to open.
	 * @param timeout
	 *            an <code>int</code> to Indicate the length of time before
	 *            giving up on opening a shell.
	 * @throws IOException
	 */
	 ...
	 * @throws IOException
	 */
	public static Shell getShell(boolean root, int timeout) throws IOException,
			TimeoutException, RootDeniedException {
	...
	/**
	 * This will return a String that represent the symlink for a specified
	 * file.
	 * <p/>
	 * 
	 * @param file
	 *            path to the file to get the Symlink for. (must have absolute
	 *            path)
	 * @return <code>String</code> a String that represent the symlink for a
	 *         specified file or an empty string if no symlink exists.
	 */
	...
	/**
	 * This will return an ArrayList of the class Symlink. The class Symlink
	 * contains the following property's: path SymplinkPath
	 * <p/>
	 * These will provide you with any Symlinks in the given path.
	 * 
	 * @param path
	 *            path to search for Symlinks.
	 * @return <code>ArrayList<Symlink></code> an ArrayList of the class
	 *         Symlink.
	 * @throws Exception
	...
	/**
	 * This will return to you a string to be used in your shell commands which
	 * will represent the valid working toolbox with correct permissions. For
	 * instance, if Busybox is available it will return "busybox", if busybox is
	 * not available but toolbox is then it will return "toolbox"
	 * 
	 * @return String that indicates the available toolbox to use for accessing
	 *         applets.
	 */
	 ...
	 */
	public static boolean installBinary(Context context, int sourceId,
	 ...
	 * @return a <code>boolean</code> which indicates whether or not the binary
	 *         already exists.
	 */
	 ...
	 * @param path
	 *            Path to the busybox binary that you want to check. (do not
	 *            include binary name)
	...
	/**
	 * This will launch the Android market looking for BusyBox, but will return
	 * the intent fired and starts the activity with startActivityForResult
	 * 
	 * @param activity
	 *            pass in your Activity
	 * @param requestCode
	 ...
	 */
	public static void runBinary(Context context, String binaryName,
			String parameter) {
	...
	/**
	 * Executes a given command with root access or without depending on the
	 * value of the boolean passed. This will also start a root shell or a
	 * standard shell without you having to open it specifically.
	 * <p/>
	 * You will still need to close the shell after you are done using the
	 * shell.
	 * 
	 * @param shell
	 *            The shell to execute the command on, this can be a root shell
	 *            or a standard shell.
	 * @param command
	...
	/**
	 * This method allows you to check whether logging is enabled. Yes, it has a
	 * goofy name, but that's to keep it as short as possible. After all writing
	 * logging calls should be painless. This method exists to save Android
	 * going through the various Java layers that are traversed any time a
	 * string is created (i.e. what you are logging)
	 * 
	 * Example usage: if(islog) { StrinbBuilder sb = new StringBuilder(); // ...
	 * // build string // ... log(sb.toString()); }
	 * 
	 * 
